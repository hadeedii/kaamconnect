...
export default function CustomerChatScreen() {
  ...
  useEffect(() => {
    ...
    const fetchProvider = async () => {
      try {
        ...
        console.error("Error fetching provider profile for chat:", error);
      }
    };
    fetchProvider();
  }, [providerId]);
  ...
  const handleSend = async () => {
    ...
    if (!text || !conversationId || !user || !providerId || !providerProfile) return;
    ...
    try {
      ...
      await setDoc(convRef, {
        ...
        providerAvatar: providerProfile.profilePicture || '',
        lastMessage: text,
  ...
  const onLongPressMessage = (m: ChatMessageItem) => {
    if (m.senderId !== user?.uid) return; // Only allow own customer messages
    setOptionsFor(m.id);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerProfile}>
          {providerProfile?.profilePicture ? (
            <Image source={{ uri: providerProfile.profilePicture }} style={styles.avatar} />
          ) : (
            <View style={styles.initialsAvatar}>
              <Text style={styles.initialsText}>
                {providerProfile?.name ? providerProfile.name.charAt(0).toUpperCase() : 'P'}
              </Text>
            </View>
          )}
          <View style={styles.headerText}>
            <Text style={styles.providerName} numberOfLines={1}>
              {providerProfile?.name || 'Loading Partner...'}
            </Text>
            <Text style={styles.providerCategory}>
              {providerProfile?.category || 'Service Specialist'}
            </Text>
          </View>
        </View>
        ...
        <TouchableOpacity 
          ...
          onPress={() => providerProfile?.phone && Linking.openURL(`tel:${providerProfile.phone}`)}
        >
      ...
      >
        ...
        {loading ? (
          ...
          >
            ...
            >
              {messages.map((m, idx) => {
                const isOwn = m.senderType === 'customer';
